...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
    ...
    if (self) {
        ...
        CGFloat  SCREEN_HEIGHT =(([UIApplication sharedApplication].statusBarHidden)? [UIScreen mainScreen].bounds.size.height : ([UIScreen mainScreen].bounds.size.height - 20.0));
        
        OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:CGRectMake(0, 0, 320, SCREEN_HEIGHT-45)
        ...
        //    // 二维码扫描背景
        if (iPhone5) {
            self.backImg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"saomiaokuang@2x.png"]];
            [self.backImg setFrame:CGRectMake((320-191)/2, 82+40, 191, 259)];
        }else{
            self.backImg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"saomiaokuang.png"]];
            [self.backImg setFrame:CGRectMake((320-191)/2, 38+40, 191, 259)];
        }
        [self.overlayView addSubview:backImg];
        [backImg release];
        
        
//        UIButton *changeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//        [changeBtn setTitle:@"从手机相册选择二维码" forState:UIControlStateNormal];
//        changeBtn.titleLabel.font = [UIFont systemFontOfSize:17.0f];
//        [changeBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
//        
//        if (iPhone5) {
//            //        [changeBtn setFrame:CGRectMake((320-196)/2, 415+88-30-36, 196, 36)];
//            [changeBtn setFrame:CGRectMake((320-196)/2, 420-30+40, 196, 36)];
//        }else{
//            //        [changeBtn setFrame:CGRectMake((320-196)/2, 415-30-36, 196, 36)];
//            [changeBtn setFrame:CGRectMake((320-196)/2, 350-30+40, 196, 36)];
//        }
        
//        [changeBtn setBackgroundImage:[UIImage imageNamed:@"btn_touming.png"] forState:UIControlStateNormal];  //btn_touming  button_red_a_on_01
//        [changeBtn setBackgroundImage:[UIImage imageNamed:@"btn_touming.png"] forState:UIControlStateHighlighted]; //button_red_a_off_01
//        [changeBtn setBackgroundImage:[UIImage imageNamed:@"btn_touming_selected.png"] forState:UIControlStateSelected];
//        [changeBtn addTarget:self action:@selector(localPhotoWithDelegate) forControlEvents:UIControlEventTouchUpInside];
//        [self.overlayView addSubview:changeBtn];
        ...
        [activity setCenter:CGPointMake(160, 160)];//指定进度轮中心点
        [activity setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleGray];//设置进度轮显示类型
        [self.view addSubview:activity];
        [activity startAnimating];
        
        //加载中。。。。。。
        activityLabel = [[UILabel alloc] initWithFrame:CGRectMake(135, 180, 60, 15)];
        activityLabel.backgroundColor = [UIColor clearColor];
        activityLabel.text = @"加载中";
...
-(void)addAnimations
{
    ...
    [self.overlayView addSubview:readline];
...
-(void)addLineImageAnimation
{
	...
	if (iPhone5) {
        translation.fromValue = [NSValue valueWithCGPoint:CGPointMake(163, 85+40)];
        translation.toValue = [NSValue valueWithCGPoint:CGPointMake(163, 240+40)];
    }else{
        
        translation.fromValue = [NSValue valueWithCGPoint:CGPointMake(163, 40+40)];
	    translation.toValue = [NSValue valueWithCGPoint:CGPointMake(163, 210+40)];
    }
...
- (void)dealloc {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
													name:@"RunFromBackground" object:nil];
    [super dealloc];
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    self.backImg.image = [UIImage imageNamed:@"saomiaokuang.png"];
...
//    backImg.image = [UIImage imageNamed:@"kuang_red.png"];
    if (!isStatusBarHidden)
        [[UIApplication sharedApplication] setStatusBarHidden:NO];
...
- (void)decoder:(Decoder *)decoder failedToDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset reason:(NSString *)reason {
    backImg.image = [UIImage imageNamed:@"saomiaokuang.png"];
    //  decoder.delegate = nil;
    [overlayView setPoints:nil];
}
