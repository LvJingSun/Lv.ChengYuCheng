...
-(void)viewWillAppear:(BOOL)animated{
    self.roomName = Appdelegate.HHcreateRoomName;
...
-(void)initRoom{
    ...
    XMPPRoom *room = [[XMPPRoom alloc] initWithRoomStorage:rosterstorage jid:[XMPPJID jidWithString:Appdelegate.HHcreateRoomName] dispatchQueue:dispatch_get_main_queue()];
    _xmppRoom = room;
    XMPPStream *stream = [XMPPServer xmppStream];
    [room activate:stream];
    
    [room joinRoomUsingNickname:[CommonUtil getValueByKey:NICK] history:nil];
    
    for (NSString *each in Appdelegate.HHinitRoomUsers) {
        
        NSLog(@"memberId = %@",each);
        
        //在创建房间完成之后直接调用[xmppRoom joinRoomUsingNickname:@"nickName" history:nil];就进入房间了，邀请好友用：
        [_xmppRoom inviteUser:[XMPPJID jidWithString:[NSString stringWithFormat:@"%@@home.cityandcity.com", each]] withMessage:@"邀请群聊"];
        
    }
    

//  [room configureRoomUsingOptions:nil jid:Appdelegate.HHcreateRoomName];
...
- (IBAction)sendButton:(id)sender {
    ...
    if (message.length > 0) {
        ...
        //由谁发送
        [mes addAttributeWithName:@"from" stringValue:[CommonUtil getValueByKey:MEMBER_ID]];
        //组合
        [mes addChild:body];
        //发送消息
...
-(void)xmppRoomDidCreate:(XMPPRoom *)sender{
    ...
    [self alertWithMessage:@"xmppRoomDidCreate"];
}

//是否已经加入房间
-(void)xmppRoomDidJoin:(XMPPRoom *)sender{
    NSLog(@"xmppRoomDidJoin");
    [self alertWithMessage:@"xmppRoomDidJoin"];
...
-(void)xmppRoomDidLeave:(XMPPRoom *)sender{
    ...
    [self alertWithMessage:@"xmppRoomDidLeave"];
...
- (void)xmppRoom:(XMPPRoom *)sender occupantDidJoin:(XMPPJID *)occupantJID withPresence:(XMPPPresence *)presence{
    ...
    [self alertWithMessage:[NSString stringWithFormat:@"occupantDidJoin----jid=%@,domain=%@,resource=%@,当前用户:%@ ,出席用户:%@,presenceType:%@",jid,domain,resource,userId,presenceFromUser,presenceType]];
...
-(void)xmppRoom:(XMPPRoom *)sender occupantDidLeave:(XMPPJID *)occupantJID withPresence:(XMPPPresence *)presence{
    ...
    [self alertWithMessage:[NSString stringWithFormat:@"用户离开，occupantDidJoin----jid=%@,domain=%@,resource=%@,当前用户:%@ ,出席用户:%@,presenceType:%@",jid,domain,resource,userId,presenceFromUser,presenceType]];
...
-(void)xmppRoom:(XMPPRoom *)sender occupantDidUpdate:(XMPPJID *)occupantJID withPresence:(XMPPPresence *)presence{
    ...
    [self alertWithMessage:[NSString stringWithFormat:@"用户加入，occupantDidJoin----jid=%@,domain=%@,resource=%@,当前用户:%@ ,出席用户:%@,presenceType:%@",jid,domain,resource,userId,presenceFromUser,presenceType]];
