...
@interface f_partyViewController ()
...
- (IBAction)faqiParty:(id)sender;

@end

@implementation f_partyViewController

@synthesize m_partyArray;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        m_partyArray = [[NSMutableArray alloc]initWithCapacity:0];
...
//    [self setRightButtonWithTitle:@"发起活动" action:@selector(launchParty)];
...
- (IBAction)faqiParty:(id)sender{
    ...
    // 个人发起聚会
    MyPartyViewController *VC = [[MyPartyViewController alloc]initWithNibName:@"MyPartyViewController" bundle:nil];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return self.m_partyArray.count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) {
        //            nib = [UINib nibWithNibName:@"BusinessCell" bundle:nil];
    ...
    if ( self.m_partyArray.count != 0 ) {
        
        NSMutableDictionary *dic = [self.m_partyArray objectAtIndex:indexPath.row];
        ...
        if (success) {
            ...
            if (pageIndex == 1) {
                if (metchantShop == nil || metchantShop.count == 0) {
                    [self.m_partyArray removeAllObjects];
                    
                    self.m_tableView.hidden = YES;
                    
                    self.m_emptyLabel.hidden = NO;
                    
                    self.m_emptyLabel.text = @"暂无会员聚会!";
                    
                    
                } else {
                    self.m_partyArray = metchantShop;
                ...
                if (metchantShop == nil || metchantShop.count == 0) {
                    ...
                    [self.m_partyArray addObjectsFromArray:metchantShop];
                }
            }
