...
- (void)viewDidLoad {
    ...
    [self setRightButtonWithTitle:@"添加地址" action:@selector(addAddress)];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    // 根据是否删除或者编辑了地址来进行刷新列表数据
    NSString *addressModify = [NSString stringWithFormat:@"%@",[CommonUtil getValueByKey:ADDRESSMODIFY]];
    
    if ( [addressModify isEqualToString:@"1"] ) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ( cell == nil ) {
        ...
        cell = (AddressListCell *)[nib objectAtIndex:0];
...
//            cell.accessoryType = UITableViewCellAccessoryNone;
        ...
        cell.m_address.frame = CGRectMake(cell.m_address.frame.origin.x, cell.m_address.frame.origin.y, WindowSizeWidth - 20, size.height);
...
//    return 60.0f;
    ...
    CGSize size = [string sizeWithFont:[UIFont systemFontOfSize:12.0f] constrainedToSize:CGSizeMake(WindowSizeWidth - 20, MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping];
...
- (void)submitorderRequest{
    ...
    [httpClient request:@"CloudMenuAddressList.ashx" parameters:param success:^(NSJSONSerialization* json) {
        ...
        if (success) {
            ...
            NSLog(@"AddressList = %@",self.m_addressList);
