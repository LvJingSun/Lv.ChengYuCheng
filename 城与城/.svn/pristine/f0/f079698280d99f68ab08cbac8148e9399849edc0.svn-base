...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    if (!self.faceView) {
        self.faceView = [[DXFaceView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 170)];
...
//        [self.faceView addSubview:m_emotionView];
    ...
    switch (tag) {
        case 0://切换状态
        {
            if (button.selected) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"addObserverremoveObserver"object:@YES];
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"addObserverremoveObserver"object:@NO];
...
- (void)deleteEmotion{
    ...
    if (self.inputTextView.text.length>0) {
        if (self.inputTextView.text.length >= 3) {
            NSString *last = [self.inputTextView.text substringFromIndex:self.inputTextView.text.length-1];
            if ([last isEqualToString:@":"]) {
                NSString *last = [self.inputTextView.text substringFromIndex:self.inputTextView.text.length-3];
                int length =3;
                if ([last rangeOfString:@"/"].location != 0  && self.inputTextView.text.length >= 4) {
                    last = [self.inputTextView.text substringFromIndex:self.inputTextView.text.length-4];
                    length = 4;
                }
                if ([last rangeOfString:@"/"].location != 0 && self.inputTextView.text.length >= 5) {
                    last = [self.inputTextView.text substringFromIndex:self.inputTextView.text.length-5];
                    length = 5;
                }
                if ([last rangeOfString:@"/"].location != 0 && self.inputTextView.text.length >= 6) {
                    last = [self.inputTextView.text substringFromIndex:self.inputTextView.text.length-6];
                    length = 6;
                }
                if ([last rangeOfString:@"/"].location == 0) {
                    self.inputTextView.text = [self.inputTextView.text substringToIndex:self.inputTextView.text.length-length];
                    [self willShowInputTextViewToHeight:[self getTextViewContentH:self.inputTextView]];
...
- (void)clickedEmotion:(NSString *)text{
    ...
    self.inputTextView.text = [self.inputTextView.text stringByAppendingString:text];
    [self willShowInputTextViewToHeight:[self getTextViewContentH:self.inputTextView]];
...
//存储草稿
- (void)didstoragedraftAction:(NSString *)nameid;
{
    if (![self.inputTextView.text isEqualToString:@""]) {

        [CommonUtil addValue:[NSString stringWithFormat:@"%@",self.inputTextView.text] andKey:[NSString stringWithFormat:@"[草稿]%@",nameid]];
