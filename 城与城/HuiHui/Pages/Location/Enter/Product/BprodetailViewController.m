...
@property (nonatomic,weak) IBOutlet UIButton *PE_isWuliu_Btn;
...
- (void)viewDidLoad
{
    ...
    self.PE_isWuliu_Btn.tag=44;
    
    [self setTitle:@"产品详情"];
    [self.ProDetailScroview setContentSize:CGSizeMake(WindowSizeWidth,1010)];
...
//    self.P_shop.text=@"无";
    ...
    [self.PE_isWuliu_Btn setImage:[UIImage imageNamed:@"comm_check_box_selected.png"] forState:UIControlStateSelected];
    [self.PE_isWuliu_Btn setImage:[UIImage imageNamed:@"comm_check_box_def.png"] forState:UIControlStateNormal];
    ...
    if ([[NSString stringWithFormat:@"%@",[self.ProDic objectForKey:@"IsWuLiu"]]isEqualToString:@"1"])
    {
        self.PE_isWuliu_Btn.selected=YES;
    }else if ([[NSString stringWithFormat:@"%@",[self.ProDic objectForKey:@"IsWuLiu"]]isEqualToString:@"0"])
    {
        self.PE_isWuliu_Btn.selected=NO;
    }

        NSLog(@"%@",self.ProDic);

        NSLog(@"%@",[self.ProDic objectForKey:@"IsWuLiu"]);
...
-(IBAction)Btnselected:(UIButton *)sender
{
    ...
    {
        if (self.PE_isWuliu_Btn.selected)
        {
            self.PE_isWuliu_Btn.selected=NO;
        }
        else
...
-(void)SetDataToDIC
{
    ...
    if (self.PE_isWuliu_Btn.selected)
    {
        [self.ALLDic setObject:@"1" forKey:@"isWuliu"];
    }
    else
    {
        [self.ALLDic setObject:@"0" forKey:@"isWuliu"];
    }
