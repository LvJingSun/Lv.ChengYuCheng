...
-(void)PutDataToServer
{
    ...
    NSString *isWuliu=[self.AllDic objectForKey:@"isWuliu"];
    key = [CommonUtil getServerKey];
    ...
    [SVProgressHUD showWithStatus:@"数据提交中"];
    [httpClient multiRequest:@"ServiceAddTest_1.ashx" parameters:param files:self.ImageDic success:^(NSJSONSerialization* json){
        BOOL success = [[json valueForKey:@"status"] boolValue];
        if (success) {
            [SVProgressHUD dismiss];
            NSString *msg=[json valueForKey:@"msg"];
