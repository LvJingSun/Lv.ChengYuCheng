...
-(void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (navigationController.viewControllers.count>=2) {
        ...
        [[viewController.view.subviews objectAtIndex:0] setFrame:CGRectMake(0, 0, WindowSizeWidth, 480-131)];
        
        selectedPan=[[UIView alloc] initWithFrame:CGRectMake(0, 480-131, WindowSizeWidth, 131)];
        UIImageView* imv=[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, WindowSizeWidth, 131)];
        ...
        [Btn_out setBackgroundImage:[UIImage imageNamed:@"button_m.png"] forState:UIControlStateNormal];
        ...
        tbv=[[UITableView alloc] initWithFrame:CGRectMake(0, 50, 90, WindowSizeWidth) style:UITableViewStylePlain];
...
-(void)updateTableView
{
    ...
    if ([[NSString stringWithFormat:@"%@",self.Allnum] isEqualToString:@"(null)"]) {
        ...
        int num = [self.Allnum intValue];
        textlabel.text= [NSString stringWithFormat:@"当前选中%i张(还可以选%i张)",pics.count,9-num-pics.count];
    }
    
    [tbv reloadData];
    
    if (pics.count>3) {
        CGFloat offsetY=tbv.contentSize.height-tbv.frame.size.height-(WindowSizeWidth-90);
