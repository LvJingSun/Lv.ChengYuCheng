...
@interface Sec_webViewController ()
...
@implementation Sec_webViewController

@synthesize m_seceryString;
...
- (void)viewDidLoad {
    ...
    self.m_seceryString = @"http://m.ly.com/scenery/?refid=40019126";
    
    [self loadWebPageWithString:[NSString stringWithFormat:@"%@",self.m_seceryString]];
...
- (void)leftClicked{
    ...
    UIActionSheet *sheet  = [[UIActionSheet alloc] initWithTitle:@"您正在查看景区门票"
                                                        delegate:self
...
//    NSLog(@"urlString = %@",string);
    ...
    if ( [string isEqualToString:@"http://m.ly.com/home/index.html"] && navigationType == UIWebViewNavigationTypeLinkClicked ) {
        ...
        UIActionSheet *sheet  = [[UIActionSheet alloc] initWithTitle:@"您正在查看景区门票信息"
                                                            delegate:self
                                                   cancelButtonTitle:@"取消"
                                              destructiveButtonTitle:nil
                                                   otherButtonTitles:@"确认返回", nil];
...
//    NSString *ljs = [self.m_webView stringByEvaluatingJavaScriptFromString:@"document.documentElement.innerHTML"];
    ...
    //    获取网页title:
//    NSString *lJs2 = [self.m_webView stringByEvaluatingJavaScriptFromString:@"document.title"];
//    
//    //    获取网页上的所有文字
...
//    [self.m_webView stringByEvaluatingJavaScriptFromString:@"$('.jalfjadsjf').find('a').attr('href','javascript: return false')"];
