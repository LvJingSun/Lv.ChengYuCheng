...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [_addButton setTitleColor:[UIColor colorWithRed:10 / 255.0 green:195 / 255.0 blue:10 / 255.0 alpha:1.0] forState:UIControlStateNormal];
        [_addButton setTitleColor:[UIColor colorWithRed:5 / 255.0 green:140 / 255.0 blue:5 / 255.0 alpha:1.0] forState:UIControlStateHighlighted];
        ...
        [_refuseButton setBackgroundColor:[UIColor colorWithRed:58 / 255.0 green:95 / 255.0 blue:80 / 255.0 alpha:1.0]];
        [_refuseButton setTitle:@"拒绝" forState:UIControlStateNormal];
        [_refuseButton setTitleColor:[UIColor colorWithRed:155 / 255.0 green:150 / 255.0 blue:0 / 255.0 alpha:1.0] forState:UIControlStateHighlighted];
...
+ (CGFloat)heightWithContent:(NSString *)content
{
    if (!content || content.length == 0) {
        ...
        CGSize size = [content sizeWithFont:[UIFont systemFontOfSize:14.0] constrainedToSize:CGSizeMake(WindowSizeWidth - 60 - 120, CGFLOAT_MAX) lineBreakMode:NSLineBreakByCharWrapping];
        return size.height > 20 ? (size.height + 40) : 60;
    }
}

- (void)setAPPlyImageViewWithPath:(NSString *)imagePath{
    
    UIImage *reSizeImage = [self.imageCache getImage:imagePath];
    if (reSizeImage != nil) {
        self.headerImageView.image = reSizeImage;
        return;
    }
    [self.headerImageView setImageWithURLRequest:[[NSURLRequest alloc] initWithURL:[NSURL URLWithString:imagePath]]
                            placeholderImage:[UIImage imageNamed:@"moren.png"]
                                     success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image){
                                         self.headerImageView.image = image;
                                         self.headerImageView.contentMode = UIViewContentModeScaleAspectFit;
                                        [self.imageCache addImage:self.headerImageView.image andUrl:imagePath];
                                     }
                                     failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error){
                                         self.headerImageView.image = [UIImage imageNamed:@"moren.png"];
