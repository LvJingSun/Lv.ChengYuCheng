...
#define kContactSize (self.view.frame.size.width - 20)/5
...
@property (strong, nonatomic) NSMutableArray *InfoSource;
...
- (instancetype)initWithGroup:(EMGroup *)chatGroup
{
    ...
    if (self) {
        ...
        _InfoSource = [[NSMutableArray alloc]init];
...
- (void)reloadDataSource
{
    ...
    //请求自己服务器接口
    [self requestSubmitFromIDS:[NSString stringWithFormat:@"%@",[self.dataSource componentsJoinedByString:@","] ]];
        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                if ([self ISnullInfo:username]) {
                    ...
                    NSString * imagePath = [NSString stringWithFormat:@"%@",[[self.IDInfoCache getInfo:username] objectForKey:@"PhotoMidUrl"]];
                    NSString * NickName = [NSString stringWithFormat:@"%@",[[self.IDInfoCache getInfo:username] objectForKey:@"NickName"]];
                    contactView.remark = NickName;
                    [contactView setImage:nil andurl:imagePath];
                    
                }else if (self.InfoSource.count ==self.dataSource.count) {
                    
                    NSMutableDictionary * dic = [self.InfoSource objectAtIndex:index];
                    [self.IDInfoCache addInfo:dic andID:[NSString stringWithFormat:@"%@",[dic objectForKey:@"MemberId"]]];
                    contactView.remark = [NSString stringWithFormat:@"%@",[dic objectForKey:@"NickName"]];
...
- (void)requestSubmitFromIDS:(NSString *)IDS{
    ...
    [httpClient request:@"GetMemberDetail.ashx" parameters:param success:^(NSJSONSerialization* json){
        ...
        if (success) {
            self.InfoSource = [json valueForKey:@"friendsInfos"];
            [self refreshScrollView];
        }
