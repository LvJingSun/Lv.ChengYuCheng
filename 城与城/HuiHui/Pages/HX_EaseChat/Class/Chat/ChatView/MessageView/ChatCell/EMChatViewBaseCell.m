...
- (id)initWithMessageModel:(MessageModel *)model reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [_headImageBtn setFrame:CGRectMake(HEAD_PADDING, CELLPADDING, HEAD_SIZE, HEAD_SIZE)];
        _headImageBtn.userInteractionEnabled = YES;
        _headImageBtn.multipleTouchEnabled = YES;
...
+ (NSString *)cellIdentifierForMessageModel:(MessageModel *)model
{
    ...
    NSLog(@"model.type = %@",model.m_type);
    
    if (model.m_type) {
        if ([model.m_type isEqualToString:@"PRO"]) {
            identifier = [identifier stringByAppendingString:@"PRO"];
        }else if ([model.m_type isEqualToString:@"WEB"]) {
            identifier = [identifier stringByAppendingString:@"WEB"];
        }else if ([model.m_type isEqualToString:@"DALI"]) {
            identifier = [identifier stringByAppendingString:@"DALI"];
        }else if ([model.m_type isEqualToString:@"MENU"]) {
            identifier = [identifier stringByAppendingString:@"MENU"];
        }else if ([model.m_type isEqualToString:@"game"]) {
            identifier = [identifier stringByAppendingString:@"game"];
        }
    }
    else
    {
