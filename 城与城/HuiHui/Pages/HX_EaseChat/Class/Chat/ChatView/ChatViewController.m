...
- (void)viewDidLoad
{
    ...
    if ([CommonUtil getValueByKey:[NSString stringWithFormat:@"[草稿]%@",_chatter]]) {
        _chatToolBar.inputTextView.text = [NSString stringWithFormat:@"%@",[CommonUtil getValueByKey:[NSString stringWithFormat:@"[草稿]%@",_chatter]]];
    }
...
-(void)RemoveNullMessageConversation
{
    ...
    if ([_chatToolBar.inputTextView.text isEqualToString:@""]) {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"[草稿]%@",_chatter]];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }else{
        [_chatToolBar didstoragedraftAction:_chatter];
    }
}
            ...
            }else
            {
              if (_isChatGroup)
              {
                  if ([self ISnullInfo:model.username]) {
                      NSString * NickName = [NSString stringWithFormat:@"%@",[[self.IDInfoCache getInfo:model.username] objectForKey:@"RealName"]];
                      ...
                      cell.headImageBtn.tag = [model.username intValue];
                      [cell.headImageBtn addTarget:self action:@selector(GoUserinfo:) forControlEvents:UIControlEventTouchUpInside];
                      ...
                      [httpClient request:@"FriendsDetail.ashx" parameters:param success:^(NSJSONSerialization* json){
                          ...
                          if (success) {
                              ...
                              NSString * NickName = [NSString stringWithFormat:@"%@",[DIC objectForKey:@"RealName"]];
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo
{
    ...
    }else if([eventName isEqualToString:kRouterEventcustomBubbleTapEventName]){
        [self chatPROCellPressed:model];
    }else if([eventName isEqualToString:kRouterEventDaliBubbleTapEventName]){
        [self chatDALICellPressed:model];
        
    }else if([eventName isEqualToString:kRouterEventMenuSuccessBubbleTapEventName]){
        
        [self zhipaiShop:model];
        
    }else if([eventName isEqualToString:kRouterEventGameBubbleTapEventName]){
        
        [self GameResultClicked:model];
        
    }
}

- (void)GameResultClicked:(MessageModel *)model {

    GameResultWebViewController *vc = [[GameResultWebViewController alloc] init];
    
    NSString *str = [NSString stringWithFormat:@"%@?memberId=%@",model.Sharestring,[CommonUtil getValueByKey:MEMBER_ID]];
...
- (void)zhipaiShop:(MessageModel *)model{
    
    NSLog(@"model = %@",model);
    // 进入指派店铺的页面
    ZhiPaiShopViewController *VC = [[ZhiPaiShopViewController alloc]initWithNibName:@"ZhiPaiShopViewController" bundle:nil];
    VC.m_orderId = [NSString stringWithFormat:@"%@",model.m_orderNO];
    [self.navigationController pushViewController:VC animated:YES];
...
//        [[UIApplication sharedApplication] openURL:url];
        ...
        VC.m_scanString = [NSString stringWithFormat:@"%@",url];
        VC.m_typeString = @"2";
        [self.navigationController pushViewController:VC animated:YES];
    }
}
...
//（自定义）产品或链接被点击
-(void)chatPROCellPressed:(MessageModel *)model
{
    [self keyBoardHidden];
    _isScrollToBottom = NO;
    [CommonUtil addValue:model.PROphotoURl andKey:@"productImage"];

    if ([model.m_type isEqualToString:@"PRO"]) {

    ProductDetailViewController *VC = [[ProductDetailViewController alloc]initWithNibName:@"ProductDetailViewController" bundle:nil];
    VC.m_productId = model.m_productId;
    //大众点评数据
    if ([model.m_FromDPId isEqualToString:@"1"]) {
        // 点击进入商品详情
        VC.m_FromDPId  =@"1";
    }else{
        // 点击进入商品详情
        VC.m_FromDPId  =@"2";
        VC.m_merchantShopId = model.m_merchantShopId;
    }
    
    [self.navigationController pushViewController:VC animated:YES];
        
    }else if ([model.m_type isEqualToString:@"WEB"]){

        WebViewController *VC = [[WebViewController alloc]initWithNibName:@"WebViewController" bundle:nil];
        VC.m_scanString = model.Sharestring;
        VC.m_typeString = @"2";
        [self.navigationController pushViewController:VC animated:YES];
...
//（自定义）代理商评分被点击
-(void)chatDALICellPressed:(MessageModel *)model
{
    [self keyBoardHidden];
    _isScrollToBottom = NO;
...
- (void)showMenuViewController:(UIView *)showInView andIndexPath:(NSIndexPath *)indexPath messageType:(MessageModel *)model
{
    if (_menuController == nil) {
        _menuController = [UIMenuController sharedMenuController];
    }
    ...
    if (model.type == eMessageBodyType_Text && (!model.m_type)) {
        [_menuController setMenuItems:@[_copyMenuItem, _deleteMenuItem]];
    }
    else{
        [_menuController setMenuItems:@[_deleteMenuItem]];
...
-(void)GoUserinfo:(id)sender
{
    ...
    if (_isChatGroup) {
        ...
        VC.m_friendId = [NSString stringWithFormat:@"%ld",(long)btn.tag];

    }else
    {
        VC.m_friendId = _chatter;
        VC.m_chatString = @"1";
    }
