...
- (void)setImage:(UIImage *)image andurl:(NSString *)imagePath;
{
    UIImageView *UImageview = [[UIImageView alloc]init];
    UIImage *reSizeImage = [self.imageCache getImage:imagePath];
    if (reSizeImage != nil) {
        _image = reSizeImage;
        _imageView.image = reSizeImage;
    }else{
        [UImageview setImageWithURLRequest:[[NSURLRequest alloc] initWithURL:[NSURL URLWithString:imagePath]]
                             placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]
                                      ...
                                      failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error){
                                          _image = [UIImage imageNamed:@"chatListCellHead.png"];
                                          _imageView.image = [UIImage imageNamed:@"chatListCellHead.png"];
