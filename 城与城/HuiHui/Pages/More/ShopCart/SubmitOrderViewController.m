...
@interface SubmitOrderViewController ()<ChooseAddressDelegate>
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.m_defaultDic = [[NSMutableDictionary alloc]initWithCapacity:0];
...
- (void)viewDidLoad
{
    ...
    if ([[self.m_items objectForKey:@"IsWuLiu"] isEqualToString:@"1"]) {
        self.m_defaultDic =[self.m_items objectForKey:@"PeiSongAddressModel"];

        if ((![[NSString stringWithFormat:@"%@",[self.m_defaultDic objectForKey:@"AddressID"]] isEqualToString:@"<null>"])) {
            
            self.m_tipLabel.hidden = YES;
            
            self.m_name.text = [NSString stringWithFormat:@"%@",[self.m_defaultDic objectForKey:@"LinkName"]];
            self.m_phone.text = [NSString stringWithFormat:@"%@",[self.m_defaultDic objectForKey:@"LinkPhone"]];
            self.m_address.text = [NSString stringWithFormat:@"%@%@%@%@",[self.m_defaultDic objectForKey:@"ProvinceName"],[self.m_defaultDic objectForKey:@"CityName"],[self.m_defaultDic objectForKey:@"AreaName"],[self.m_defaultDic objectForKey:@"Address"]];
        }else
        {
    ...
    {
        [self.m_dingdanV setFrame:CGRectMake(0, self.m_dingdanV.frame.origin.y-65, self.m_dingdanV.frame.size.width, self.m_dingdanV.frame.size.height)];
...
- (IBAction)choseAddress:(id)sender {
    ...
    VC.m_addressId = [NSString stringWithFormat:@"%@",[[self.m_items objectForKey:@"PeiSongAddressModel"] objectForKey:@"AddressID"]];
    VC.delegate = self;
...
- (IBAction)submitOrder:(id)sender {
    ...
    NSDictionary *param = [NSDictionary dictionaryWithObjectsAndKeys:
                           ...
                           [NSString stringWithFormat:@"%li",(long)self.m_acount],@"quantity",
...
//    NSString *totalPrice = [NSString stringWithFormat:@"%.2f",self.m_acount * [[NSString stringWithFormat:@"%@",[self.m_items objectForKey:@"Price"]] floatValue]];
    ...
    NSDictionary *param = [NSDictionary dictionaryWithObjectsAndKeys:
                           memberId,     @"memberId",
                           key,   @"key",
                           [NSString stringWithFormat:@"%li",(long)self.m_acount],@"amount",
                           [NSString stringWithFormat:@"%@",[self.m_items objectForKey:@"ServiceID"]],@"serviceId",
                           @"2",@"type",
                           self.m_name.text,@"linkName",
                           self.m_phone.text,@"linkPhone",
                           self.m_address.text,@"linkAddress",
                           nil];
    [SVProgressHUD showWithStatus:@"数据加载中"];
    [httpClient request:@"Buy_2.ashx" parameters:param success:^(NSJSONSerialization* json) {
...
//            NSString *msg = [json valueForKey:@"msg"];
            ...
            NSMutableDictionary *dic = [[NSMutableDictionary alloc]initWithObjectsAndKeys:self.m_unitPrice.text,@"Price",[NSString stringWithFormat:@"%li",(long)self.m_acount],@"quantity",[NSString stringWithFormat:@"%@",[self.m_items objectForKey:@"ServiceID"]],@"svcId",[self.m_items objectForKey:@"SvcSimpleName"],@"SvcSimpleName", nil];
