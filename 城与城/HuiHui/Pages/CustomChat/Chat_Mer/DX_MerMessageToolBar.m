...
@interface DX_MerMessageToolBar()<UITextViewDelegate, DXFaceDelegate>
{
...
@property (strong, nonatomic) UIButton *merstyleButton;//更大切换（用于商户的）
@property (strong, nonatomic) UIView *merstyleview;
...
@implementation DX_MerMessageToolBar
...
- (void)setupSubviews
{
    ...
    self.merstyleButton = [[UIButton alloc] initWithFrame:CGRectMake(2, 8, 35, 35)];
    self.merstyleButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [self.merstyleButton setImage:[UIImage imageNamed:@"chatDetail_selected.png"] forState:UIControlStateNormal];
    [self.merstyleButton setImage:[UIImage imageNamed:@"jianpan_normal.png"] forState:UIControlStateSelected];
    [self.merstyleButton addTarget:self action:@selector(BigAction:) forControlEvents:UIControlEventTouchUpInside];
    self.merstyleButton.tag = 99;

    //转变输入样式
    self.styleChangeButton = [[UIButton alloc] initWithFrame:CGRectMake(kHorizontalPadding + 32, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    ...
    [self.toolbarView addSubview:self.merstyleButton];
     ...
     **
     以下是切换
     */
    self.merstyleview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, WindowSizeWidth, 46)];
    self.merstyleview.backgroundColor = [UIColor whiteColor];
    UIButton * BTN1 = [UIButton buttonWithType:UIButtonTypeCustom];
    BTN1.frame = CGRectMake(2, self.styleChangeButton.frame.origin.y +1, 35, 35);
    [BTN1 setImage:[UIImage imageNamed:@"detail_selected.png"] forState:UIControlStateNormal];
    [BTN1 addTarget:self action:@selector(BigAction:) forControlEvents:UIControlEventTouchUpInside];
    BTN1.tag = 1;
    
    
    UIButton * BTN2 =[UIButton buttonWithType:UIButtonTypeCustom];
    BTN2.frame = CGRectMake(BTN1.frame.size.width +2+1, 0, (WindowSizeWidth-BTN1.frame.size.width -2-1)/3, 46);
    ...
    [BTN2 addTarget:self action:@selector(BigAction:) forControlEvents:UIControlEventTouchUpInside];
    BTN2.tag = 2;

    
    UIButton * BTN3 =[UIButton buttonWithType:UIButtonTypeCustom];
    BTN3.frame = CGRectMake(BTN2.frame.origin.x +BTN2.frame.size.width,0, (WindowSizeWidth-BTN1.frame.size.width -2-1)/3, 46);
    BTN3.backgroundColor = [UIColor clearColor];
    [BTN3 setTitle: @"店铺" forState: UIControlStateNormal];
    ...
    [BTN3 addTarget:self action:@selector(BigAction:) forControlEvents:UIControlEventTouchUpInside];
    BTN3.tag = 3;

    
    UIButton * BTN4 =[UIButton buttonWithType:UIButtonTypeCustom];
    BTN4.frame = CGRectMake(BTN3.frame.origin.x+BTN3.frame.size.width,0, (WindowSizeWidth-BTN1.frame.size.width -2-1)/3, 46);
    BTN4.backgroundColor = [UIColor clearColor];
    [BTN4 setTitle: @"关于我们" forState: UIControlStateNormal];
    ...
    [BTN4 addTarget:self action:@selector(BigAction:) forControlEvents:UIControlEventTouchUpInside];
    BTN4.tag = 4;


    UIImageView * iamge1 = [[UIImageView alloc]initWithFrame:CGRectMake(40, 0, 1, 44)];
    iamge1.backgroundColor = [UIColor lightGrayColor];
    UIImageView * iamge2 = [[UIImageView alloc]initWithFrame:CGRectMake( 0, 0, WindowSizeWidth, 1)];
    iamge2.backgroundColor = [UIColor lightGrayColor];
    UIImageView * iamge3 = [[UIImageView alloc]initWithFrame:CGRectMake(BTN2.frame.origin.x+BTN2.frame.size.width, 0, 1, 44)];
    iamge3.backgroundColor = [UIColor lightGrayColor];
    UIImageView * iamge4 = [[UIImageView alloc]initWithFrame:CGRectMake(BTN3.frame.origin.x+BTN3.frame.size.width, 0, 1, 44)];
    iamge4.backgroundColor = [UIColor lightGrayColor];
    UIImageView * iamge5 = [[UIImageView alloc]initWithFrame:CGRectMake(iamge3.frame.origin.x -25, 14, 22, 32)];
    iamge5.image = [UIImage imageNamed:@"abs__spinner_ab_default_holo_dark.png"];
    UIImageView * iamge6 = [[UIImageView alloc]initWithFrame:CGRectMake(iamge4.frame.origin.x -25, 14, 22, 32)];
    iamge6.image = [UIImage imageNamed:@"abs__spinner_ab_default_holo_dark.png"];
    UIImageView * iamge7 = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) -25, 16, 22, 32)];
    iamge7.image = [UIImage imageNamed:@"abs__spinner_ab_default_holo_dark.png"];

    [self.merstyleview addSubview:BTN1];
    [self.merstyleview addSubview:BTN2];
    [self.merstyleview addSubview:BTN3];
    [self.merstyleview addSubview:BTN4];
    [self.merstyleview addSubview:iamge1];
    [self.merstyleview addSubview:iamge2];
    [self.merstyleview addSubview:iamge3];
    [self.merstyleview addSubview:iamge4];
    [self.merstyleview addSubview:iamge5];
    [self.merstyleview addSubview:iamge6];
    [self.merstyleview addSubview:iamge7];

    [self.toolbarView addSubview:self.merstyleview];
}
...
-(void)BigAction:(id)sender
{
    UIButton *button = (UIButton *)sender;

    switch (button.tag) {
        case 1:{
            self.merstyleview.hidden = YES;
        }
            break;
        case 2:{
            // 进入商品列表
            [[NSNotificationCenter defaultCenter] postNotificationName:@"Mer_BigAction"object:@"1"];
            
        }
            break;
        case 3:{
            // 进入店铺列表
            [[NSNotificationCenter defaultCenter] postNotificationName:@"Mer_BigAction"object:@"2"];
        ...
        case 4:{
            // 进入关于我们的页面
            [[NSNotificationCenter defaultCenter] postNotificationName:@"Mer_BigAction"object:@"3"];
            
        }
            break;
        case 99:{
            [self endEditing:YES];
            self.merstyleview.hidden = NO;
        }
            break;
