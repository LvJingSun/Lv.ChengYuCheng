...
#import "HHImagePrefetcher.h"
#import "HHImageManager.h"

@interface HHImagePrefetcher ()

@property (strong, nonatomic) HHImageManager *manager;
...
@implementation HHImagePrefetcher

+ (HHImagePrefetcher *)sharedImagePrefetcher
{
...
- (id)init
{
    if ((self = [super init]))
    {
        _manager = HHImageManager.new;
        _options = HHImageLowPriority;
...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    self.requestedCount++;
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, HHImageType cacheType, BOOL finished)
    {
        if (!finished) return;
        self.finishedCount++;
