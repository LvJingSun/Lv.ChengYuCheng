...
- (id)initWithNamespace:(NSString *)ns
{
    if ((self = [super init]))
    {
        NSString *fullNamespace = [@"com.hackemist.HHImageCache." stringByAppendingString:ns];

        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.hackemist.HHImageCache", DISPATCH_QUEUE_SERIAL);
...
- (NSOperation *)queryDiskCacheForKey:(NSString *)key done:(void (^)(UIImage *image, HHImageType cacheType))doneBlock
{
    ...
    if (image)
    {
        doneBlock(image, HHImageTypeMemory);
        return nil;
    }
    ...
    dispatch_async(self.ioQueue, ^
    {
        ...
        @autoreleasepool
        {
            ...
            {
                doneBlock(diskImage, HHImageTypeDisk);
            });
        }
    });
