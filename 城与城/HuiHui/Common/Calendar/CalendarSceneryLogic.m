...
@interface CalendarSceneryLogic ()
{
    NSDate *today;//今天的日期
    NSDate *before;//之后的日期
    NSDate *select;//选择的日期
...
@implementation CalendarSceneryLogic
...
//    NSLog(@"array = %@",array);
    ...
    self.m_array = [[NSMutableArray alloc]initWithCapacity:0];
    ...
    for (int i = 0; i < months; i++) {
        ...
        self.m_array = [array objectAtIndex:i];
...
- (void)calculateDaysInCurrentMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (int i = 0; i < daysCount; i++) {
        ...
        if (self.m_array.count != 0 ) {
            ...
            NSDictionary *dic = [self.m_array objectAtIndex:i];
            
            // 判断值为0的时候显示为空，表示不可选择该日期
            NSString *string = [NSString stringWithFormat:@"%@",[dic objectForKey:@"tcPrice"]];
            ...
            if ( [string isEqualToString:@"0"] ) {
                ...
                self.m_string = [NSString stringWithFormat:@"￥%@",[dic objectForKey:@"tcPrice"]];
        ...
        CalendarDayModel *calendarDay = [CalendarDayModel calendarDayWithYear:components.year month:components.month day:i + 1];
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    //1.1元旦
        ...
        //12.24平安夜
    }else if (calendarDay.month == 12 &&
              calendarDay.day == 24){
        calendarDay.holiday = @"平安夜";
        
        //12.25圣诞节
    }else if (calendarDay.month == 12 &&
              calendarDay.day == 25){
        calendarDay.holiday = @"圣诞节";
...
-(void)LunarForSolarYear:(CalendarDayModel *)calendarDay{
    ...
    calendarDay.Chinese_calendar = [NSString stringWithFormat:@"%@",self.m_string];
