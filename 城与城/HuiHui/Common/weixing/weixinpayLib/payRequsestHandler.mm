...
//    NSString *order_name    = @"V3支付测试";
    ...
    NSString *order_name = [NSString stringWithFormat:@"%@",[CommonUtil getValueByKey:WEIXIN_NAME]];
    
    //订单金额,单位（分）
//    NSString *order_price   = @"1";//1分钱测试
    
    NSString *price = [NSString stringWithFormat:@"%@",[CommonUtil getValueByKey:WEIXIN_PRICE]];
    
    float totalPrice = [price floatValue] * 100;
    
    NSString *order_price = [NSString stringWithFormat:@"%.f",totalPrice];
    
    // 根据保存的值来判断回调地址
    NSString *typeString = [NSString stringWithFormat:@"%@",[CommonUtil getValueByKey:WEIXIN_PAYTYPE]];
    
    NSString *notifyUrl = @"";
    
    if ( [typeString isEqualToString:@"1"] ) {
        // 表示商品购买
        notifyUrl = @"http://wx.cityandcity.com/wxpays/mobilenotifysvc_url.aspx";
        
    }else if ( [typeString isEqualToString:@"2"] ){
        // 表示代理商
        notifyUrl = @"http://wx.cityandcity.com/wxpays/mobilenotifyccdl_url.aspx";
        
    }else if ( [typeString isEqualToString:@"3"] ){
        // 表示是点单购买
        notifyUrl = @"http://wx.cityandcity.com/wxpays/mobilecloudmenupay_url.aspx";
    ...
    NSString *orderno   = [NSString stringWithFormat:@"%@",[CommonUtil getValueByKey:WEIXIN_OREDENO]];
    ...
    [packageParams setObject: orderno           forKey:@"attach"];
